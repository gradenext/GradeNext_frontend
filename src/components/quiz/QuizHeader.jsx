  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="text-xl capitalize font-bold bg-gradient-to-r from-purple-600 to-blue-600 text-transparent bg-clip-text">
          {/* {config.displayNames[currentTopic]} (Level:{" "}
							{difficultyLevel + 1}) */}
          {quizQuestion?.progress?.current_topic?.split("_").join(" ")} (Level:
          {quizQuestion?.progress?.current_level?.split("_").join(" ")})
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mt-2">
        ...
        <StreakDisplay currentStreak={current_streak} bestStreak={max_streak} />
      </div>
    </motion.div>
  );
};
