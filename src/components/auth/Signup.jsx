...
import { UserPlus, User, Users, School, Mail, Lock, Loader2, Phone } from 'lucide-react';
...
const Signup = () => {
  ...
  const validateStep = () => {
    if (step === 1) {
      ...
      if (formData.courses.length === 0) return "At least one course must be selected";
    } else if (step === 2) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.error || 'Registration failed. Please check all fields and try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          <div className="w-full max-w-2xl border-4 border-purple-300 shadow-xl bg-white rounded-3xl overflow-hidden">
            <div className="bg-gradient-to-r from-purple-100 to-blue-100 p-6">
              ...
              <p className="text-center text-purple-600 mt-2">Step {step} of 3</p>
